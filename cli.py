...
@click.command("inference", help="머신러닝 모데을 사용하여 추론을 수행")
@click.option("--cylinders", required=True, type=int)
@click.option("--displacement", required=True, type=float) # 배기량
@click.option("--horsepower", required=True, type=int)
@click.option("--weight", required=True, type=int)
@click.option("--acceleration", required=True, type=float)  # 가속능력 제로백
